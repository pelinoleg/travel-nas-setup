...
2. `ssh oleg@<host>.local` — пароль НЕ хранить в репо (публичный). Передаётся через env `SSHPASS` локально (см. `.claude/settings.local.json` или спроси юзера). Hostname задаёт юзер в Pi Imager при прошивке — НЕ хардкодь `pi.local` в коде.
